...
func _ready():
	...
	self.attack_collision = Collision.create_new_shape_with_modified_extents(self.collision_node, self.base_size.x * 1, 0)
	self.attack_padding = Vector2(base_size.x * 1,0)
