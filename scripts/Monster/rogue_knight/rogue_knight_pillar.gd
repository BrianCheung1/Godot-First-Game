...
class_name RogueKnightMeleePillar

@export var rogue_knight: RogueKnight
var logger = Logger.new("[rogue knight melee pillar]")
...
static func create(parent: RogueKnight, target_loc: Vector2) -> RogueKnightMeleePillar:
	var res = load("res://scenes/monsters/rogue_knight/rogue_knight_melee_pillar.tscn")
	var node: RogueKnightMeleePillar = res.instantiate()
